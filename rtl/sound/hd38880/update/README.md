...
## 4f. Bit 6 resolved: an unused repeat flag

The TMS5220 comparison put the repeat bit immediately after energy and before
pitch - exactly where bit 6 sits in the HD38880 frame. That made the TI-style
"repeat frame" (amp + repeat + pitch only, reusing the previous k's) a live
hypothesis, and it would have explained why INT2 sets repeat-enable while no
repeat mechanism was ever found in the data.

**Result: bit 6 is 0 in every valid frame of both games - 905 Vanguard frames,
761 Fantasy frames, 1666 total, zero exceptions.**

So the repeat mechanism exists in the frame layout (the bit is allocated, and
INT2 enables it) but the encoder never emitted a single repeat frame. A repeat
reading and a stop reading therefore produce identical parses; the only
measurable difference is a slightly tighter Fantasy duration spread under the
repeat rule (0.074 vs 0.142), which follows from not terminating early.

Two earlier claims are corrected by this:
- The "18% of frames have bit 6 set" figure came from the pre-fix desynchronised
  parse and was noise.
- "Fantasy terminators have the repeat bit set" was an artifact: the terminator
  is an all-ones pattern (`1111111111111`), so bit 6 is 1 only as part of amp=63.
  **The amp=63 all-ones code is the sole terminator.**

The 49th bit is nonetheless real, not a phantom: bit-agreement autocorrelation,
which makes no field assumptions, peaks decisively at T=49 in both games
(0.681 Vanguard, 0.712 Fantasy) with the nearest competitor at 0.515/0.561.

**For RTL:** decode bit 6 as a repeat flag for completeness (reuse previous k's,
frame length 13 bits), but no known ROM exercises it. Treat amp==63 as the stop.
...
- INT2 bit 1 ("repeat enable") is set by both games, and bit 6 of the frame is
  the repeat flag by position (matching the TMS5220 layout), but it is 0 in all
  1666 valid frames — the feature is allocated and enabled but never used. See 4f.
