...
//
// SPEECH-VANGUARD-MIX-2026-08-23: Vanguard's speech was still buried in the
// mix. Measured levels here: bomb one-shot bursts near FULL SCALE (+-32k,
// snk6502_noise.v drives {lfsr[11:0],4'b0}), SHOT A/B run CONTINUOUSLY at
// +-6000/+-3000 while the player holds fire, while speech peaks ~+-8.5k with
// a far lower average. MAME has the same imbalance - it routes Vanguard
// speech samples at 0.25 vs custom 0.50 / SHOT A 0.50 (snk6502_a.cpp:691-730)
// - and the user confirmed MAME Vanguard speech is inaudible without boosting
// the samples channel ~300% and reducing everything else. So, VANGUARD ONLY:
// speech gain x4 (was x2) with output saturation, and the three noise sources
// (SHOT A, SHOT B, BOMB) halved below. Music tones untouched (already the
// quietest element); Fantasy path unchanged (its speech level was confirmed
// fine). TUNABLE: the <<< 2 and the three >>> 1 taps below.
// Original: wire signed [15:0] speech_audio_ext    = {speech_audio[14], speech_audio};
// Original: wire signed [15:0] speech_audio_scaled = speech_audio_ext <<< 1;
wire signed [17:0] speech_audio_w18 = {{3{speech_audio[14]}}, speech_audio};
wire signed [17:0] speech_audio_amp = game_vg ? (speech_audio_w18 <<< 2)
                                              : (speech_audio_w18 <<< 1);
wire signed [15:0] speech_audio_scaled =
      (speech_audio_amp >  18'sd32767) ? 16'sd32767
    : (speech_audio_amp < -18'sd32768) ? 16'sh8000
    : speech_audio_amp[15:0];

// SPEECH-VANGUARD-MIX-2026-08-23: Vanguard-only -6 dB on the maskers.
wire signed [15:0] noise_mix = game_vg ? (noise_audio >>> 1) : noise_audio;
wire signed [15:0] sn_lo_mix = game_vg ? (sn_lo_audio >>> 1) : sn_lo_audio;
wire signed [15:0] sn_hi_mix = game_vg ? (sn_hi_audio >>> 1) : sn_hi_audio;
...
// widens with it.
// SPEECH-VANGUARD-MIX-2026-08-23: noise/sn legs go through the game-aware
// _mix taps above (Vanguard -6 dB; identity for every other game).
// Original: wire signed [19:0] audio_mix3 = $signed(snd_audio) + $signed(noise_audio)
// Original:                               + $signed(speech_audio_scaled)
// Original:                               + $signed(sn_lo_audio) + $signed(sn_hi_audio);
wire signed [19:0] audio_mix3 = $signed(snd_audio) + $signed(noise_mix)
                              + $signed(speech_audio_scaled)
                              + $signed(sn_lo_mix) + $signed(sn_hi_mix);
