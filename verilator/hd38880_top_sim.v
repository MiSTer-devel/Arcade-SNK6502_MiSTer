//=============================================================================
// hd38880_top_sim.v  -- SIM-ONLY COPY of rtl/sound/hd38880/hd38880_top.v
//
// Real RTL is UNTOUCHED. Two differences from the real file, both deliberate:
//
//  1. The DIAG-REVERT-2026-08-01 forced-phrase-0 kick is REVERTED here (the
//     real muxed ctrl_start / ctrl_adset are restored) so this bench exercises
//     the REAL CPU -> hd38880_ctrl -> unpack path end to end. That is the
//     whole point of driving it with the actual $3400 write stream recovered
//     from the MAME trace + disassembly.
//     A `force_start` / `force_addr` input is provided as well so the bench
//     can ALSO reproduce the DIAG behaviour on demand and settle whether the
//     diagnostic's own forcing logic was sound (an open question in
//     Claude/hd38880_speech_integration_20260801.md).
//
//  2. Debug observation outputs (dbg_*) are added. Pure taps, no logic change.
//
// Everything else is byte-identical in behaviour to the real hd38880_top.v.
//=============================================================================

module hd38880_top_sim #(
    parameter CLK_HZ = 11_289_000
)(
    input  wire        clk,
    input  wire        reset,
    input  wire        pause,

    input  wire        host_wr,
    input  wire  [5:0] host_din,
    input  wire        game_is_fantasy,

    // Bench-only: reproduce the DIAG forced start when force_en=1
    input  wire        force_en,
    input  wire        force_start,
    input  wire [19:0] force_addr,

    output wire [12:0] rom_addr,
    input  wire [7:0]  rom_data,

    output wire signed [14:0] audio_out,

    // ---- debug taps ---------------------------------------------------------
    output wire [19:0] dbg_adset_addr,
    output wire        dbg_ctrl_start,
    output wire [3:0]  dbg_phrase,
    output wire        dbg_phrase_ok,
    output wire [3:0]  dbg_int1,
    output wire [3:0]  dbg_int2,
    output wire        dbg_frame_valid,
    output wire        dbg_frame_ack,
    output wire        dbg_frame_we,
    output wire        dbg_unpack_done,
    output wire        dbg_rom_rd,
    output wire [19:0] dbg_unpack_rom_addr,
    output wire        dbg_f_silent,
    output wire [5:0]  dbg_f_amp,
    output wire [5:0]  dbg_f_pitch,
    output wire [6:0]  dbg_f_k1,
    output wire [4:0]  dbg_f_k2,
    output wire [2:0]  dbg_f_k3, dbg_f_k4, dbg_f_k5, dbg_f_k6,
    output wire [2:0]  dbg_f_k7, dbg_f_k8, dbg_f_k9, dbg_f_k10,
    output wire [7:0]  dbg_dec_amp,
    output wire [7:0]  dbg_dec_pitch,
    output wire        dbg_audio_stb
);

    // ---- Command decode: one instance per phrase table, muxed by game -----
    wire        start_vg, stop_vg, phrase_ok_vg;
    wire [19:0] adset_addr_vg;
    wire [3:0]  int1_cfg_vg, int2_cfg_vg, syspd_vg, phrase_vg;

    hd38880_ctrl #(.GAME(0)) ctrl_vanguard (
        .clk(clk), .rst(reset),
        .wr(host_wr), .din(host_din),
        .start(start_vg), .stop(stop_vg),
        .phrase(phrase_vg), .phrase_ok(phrase_ok_vg), .adset_addr(adset_addr_vg),
        .int1_cfg(int1_cfg_vg), .int2_cfg(int2_cfg_vg), .syspd(syspd_vg)
    );

    wire        start_fy, stop_fy, phrase_ok_fy;
    wire [19:0] adset_addr_fy;
    wire [3:0]  int1_cfg_fy, int2_cfg_fy, syspd_fy, phrase_fy;

    hd38880_ctrl #(.GAME(1)) ctrl_fantasy (
        .clk(clk), .rst(reset),
        .wr(host_wr), .din(host_din),
        .start(start_fy), .stop(stop_fy),
        .phrase(phrase_fy), .phrase_ok(phrase_ok_fy), .adset_addr(adset_addr_fy),
        .int1_cfg(int1_cfg_fy), .int2_cfg(int2_cfg_fy), .syspd(syspd_fy)
    );

    // REVERTED DIAG: real muxed signals (this is the original real-RTL line).
    wire        real_start = game_is_fantasy ? start_fy      : start_vg;
    wire [19:0] real_adset = game_is_fantasy ? adset_addr_fy : adset_addr_vg;

    wire        ctrl_start = force_en ? force_start : real_start;
    wire [19:0] ctrl_adset = force_en ? force_addr  : real_adset;

    wire        ctrl_stop  = game_is_fantasy ? stop_fy       : stop_vg;
    wire [3:0]  ctrl_int1  = game_is_fantasy ? int1_cfg_fy   : int1_cfg_vg;
    wire [3:0]  ctrl_int2  = game_is_fantasy ? int2_cfg_fy   : int2_cfg_vg;

    // ---- Bitstream unpacker -------------------------------------------------
    wire        unpack_done;
    wire [19:0] unpack_rom_addr;
    reg         unpack_rom_valid;
    wire        unpack_rom_rd;

    wire        frame_valid;
    reg         frame_ack;
    wire        f_silent;
    wire [5:0]  f_amp, f_pitch;
    wire [6:0]  f_k1;
    wire [4:0]  f_k2;
    wire [2:0]  f_k3, f_k4, f_k5, f_k6, f_k7, f_k8, f_k9, f_k10;

    hd38880_unpack unpack (
        .clk(clk), .rst(reset),
        .start(ctrl_start), .start_addr(ctrl_adset),
        .abort(ctrl_stop), .done(unpack_done),
        .rom_addr(unpack_rom_addr), .rom_data(rom_data),
        .rom_rd(unpack_rom_rd), .rom_valid(unpack_rom_valid),
        .frame_valid(frame_valid), .frame_ack(frame_ack),
        .f_silent(f_silent), .f_amp(f_amp), .f_pitch(f_pitch),
        .f_k1(f_k1), .f_k2(f_k2), .f_k3(f_k3), .f_k4(f_k4), .f_k5(f_k5),
        .f_k6(f_k6), .f_k7(f_k7), .f_k8(f_k8), .f_k9(f_k9), .f_k10(f_k10)
    );

    always @(posedge clk) unpack_rom_valid <= unpack_rom_rd;

    wire [19:0] rom_addr_rel = unpack_rom_addr - 20'h04000;
    assign rom_addr = rom_addr_rel[12:0];

    // ---- Decode ROM ---------------------------------------------------------
    wire [7:0]        dec_amp, dec_pitch;
    wire signed [9:0] dec_k1, dec_k2, dec_k3, dec_k4, dec_k5,
                       dec_k6, dec_k7, dec_k8, dec_k9, dec_k10;

    hd38880_dectab dectab (
        .amp_code(f_amp), .pitch_code(f_pitch),
        .k1_code(f_k1), .k2_code(f_k2), .k3_code(f_k3), .k4_code(f_k4),
        .k5_code(f_k5), .k6_code(f_k6), .k7_code(f_k7), .k8_code(f_k8),
        .k9_code(f_k9), .k10_code(f_k10),
        .amp_out(dec_amp), .pitch_out(dec_pitch),
        .k1_out(dec_k1), .k2_out(dec_k2), .k3_out(dec_k3), .k4_out(dec_k4),
        .k5_out(dec_k5), .k6_out(dec_k6), .k7_out(dec_k7), .k8_out(dec_k8),
        .k9_out(dec_k9), .k10_out(dec_k10)
    );

    // ---- 20 ms frame pacing --------------------------------------------------
    localparam integer FRAME_DIV = (CLK_HZ / 1000) * 20;
    reg [23:0] frame_cnt;
    wire       frame_tick = (frame_cnt == FRAME_DIV - 1);

    always @(posedge clk) begin
        if (reset)      frame_cnt <= 24'd0;
        else if (pause) frame_cnt <= frame_cnt;
        else if (frame_tick) frame_cnt <= 24'd0;
        else            frame_cnt <= frame_cnt + 24'd1;
    end

    reg               frame_we;
    reg  [7:0]         amp_lat, pitch_lat;
    reg  signed [9:0]  k1_lat, k2_lat, k3_lat, k4_lat, k5_lat,
                        k6_lat, k7_lat, k8_lat, k9_lat, k10_lat;
    reg                done_d;

    always @(posedge clk) begin
        frame_ack <= 1'b0;
        frame_we  <= 1'b0;
        done_d    <= unpack_done;

        if (reset) begin
            amp_lat <= 8'd0; pitch_lat <= 8'd0;
            k1_lat <= 10'sd0; k2_lat <= 10'sd0; k3_lat <= 10'sd0; k4_lat <= 10'sd0;
            k5_lat <= 10'sd0; k6_lat <= 10'sd0; k7_lat <= 10'sd0; k8_lat <= 10'sd0;
            k9_lat <= 10'sd0; k10_lat <= 10'sd0;
        end
        else if (!pause && frame_tick && frame_valid) begin
            frame_ack <= 1'b1;
            frame_we  <= 1'b1;
            amp_lat   <= dec_amp;
            pitch_lat <= dec_pitch;
            k1_lat  <= dec_k1;  k2_lat <= dec_k2;  k3_lat <= dec_k3;  k4_lat <= dec_k4;
            k5_lat  <= dec_k5;  k6_lat <= dec_k6;  k7_lat <= dec_k7;  k8_lat <= dec_k8;
            k9_lat  <= dec_k9;  k10_lat <= dec_k10;
        end
        else if (unpack_done && !done_d) begin
            frame_we <= 1'b1;
            amp_lat  <= 8'd0;
        end
    end

    wire audio_stb_w;

    parcor_lattice #(.CLK_HZ(CLK_HZ)) lattice (
        .clk(clk), .rst(reset), .pause(pause),
        .k1_in(k1_lat),  .k2_in(k2_lat),  .k3_in(k3_lat),  .k4_in(k4_lat),  .k5_in(k5_lat),
        .k6_in(k6_lat),  .k7_in(k7_lat),  .k8_in(k8_lat),  .k9_in(k9_lat), .k10_in(k10_lat),
        .amp_in(amp_lat), .pitch_in(pitch_lat), .frame_we(frame_we),
        .tri_src(ctrl_int1[3]), .loss_en(ctrl_int1[2]), .ten_stage(ctrl_int2[0]),
        .ext_pitch(8'd0), .ext_pitch_en(1'b0),
        .audio_out(audio_out), .audio_stb(audio_stb_w)
    );

    // ---- debug taps ---------------------------------------------------------
    assign dbg_adset_addr      = real_adset;
    assign dbg_ctrl_start      = ctrl_start;
    assign dbg_phrase          = game_is_fantasy ? phrase_fy    : phrase_vg;
    assign dbg_phrase_ok       = game_is_fantasy ? phrase_ok_fy : phrase_ok_vg;
    assign dbg_int1            = ctrl_int1;
    assign dbg_int2            = ctrl_int2;
    assign dbg_frame_valid     = frame_valid;
    assign dbg_frame_ack       = frame_ack;
    assign dbg_frame_we        = frame_we;
    assign dbg_unpack_done     = unpack_done;
    assign dbg_rom_rd          = unpack_rom_rd;
    assign dbg_unpack_rom_addr = unpack_rom_addr;
    assign dbg_f_silent        = f_silent;
    assign dbg_f_amp           = f_amp;
    assign dbg_f_pitch         = f_pitch;
    assign dbg_f_k1            = f_k1;
    assign dbg_f_k2            = f_k2;
    assign dbg_f_k3            = f_k3;
    assign dbg_f_k4            = f_k4;
    assign dbg_f_k5            = f_k5;
    assign dbg_f_k6            = f_k6;
    assign dbg_f_k7            = f_k7;
    assign dbg_f_k8            = f_k8;
    assign dbg_f_k9            = f_k9;
    assign dbg_f_k10           = f_k10;
    assign dbg_dec_amp         = dec_amp;
    assign dbg_dec_pitch       = dec_pitch;
    assign dbg_audio_stb       = audio_stb_w;

endmodule
